    ...
    <div className="w-full">
      ...
      <div className="flex flex-col justify-center items-center">
        <h2 className="text-2xl">
          {selectedDay === weather.days[0] ? "Today" : selectedDay.datetime}
        </h2>
        <div className="flex items-center gap-5 mt-2">
          <div className="flex items-center gap-2">
            {getHour(selectedDay.sunrise)}
            <FiSunrise className="text-lg text-amber-300" />
          </div>
          <div className="flex items-center gap-2">
            {getHour(selectedDay.sunset)}
            <FiSunset className="text-lg text-amber-600" />
          </div>
        </div>
          ...
          <table className="min-w-full table-fixed border border-separate rounded-md shadow-lg mt-5 mb-5">
            ...
            <tbody>
              {selectedDay.hours.map((hour, index) => (
                <tr
                  key={index}
                  className="hover:font-bold hover:shadow-2xl hover:text-amber-200"
                >
                  <td className="ml-5 flex items-center gap-1">
                    {getHour(hour.datetime)}{" "}
                    {generateIcon(hour.cloudcover, hour.precipprob)}
                  </td>
